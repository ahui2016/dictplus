...
const Form = cc('form', {
  ...
  children: [
    util.create_item(
      DictplusAddrInput,
      'Dictplus Address',
      'dictplus 的默认网址，修改后下次启动时生效。'
    ),
    util.create_item(
      LocaltagsAddrInput,
      'Localtags Address',
      'localtags 的网址，详细说明请看 README.md 中的 "插图" 部分。'
    ),
    m('div')
      .addClass('mb-3')
      .append(
...
const download_db_area = m('div').append(
...
$('#root').append(
  ...
  m(Alerts),
  m(Form).hide(),
  download_db_area.addClass('my-5')
);
...
function init() {
  util.ajax(
    {method: 'GET', url: '/api/get-settings', alerts: Alerts},
    resp => {
